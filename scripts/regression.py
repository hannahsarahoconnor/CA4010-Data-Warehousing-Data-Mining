...
class PreprocessingPPR:
    ...
    def _remove_unnecessary_rows(df):
        ...
        df = df[(df['property_description'] == 'Second-Hand Dwelling house /Apartment') | (
                 df['property_description'] == 'New Dwelling house /Apartment')]
        return df
    ...
    def num_processing(self):
        ...
        self.df = self.df.drop(['County', 'Date of pip install catboost', 'Property Size Description', 'Description of Property',
...
def main():
    ...
    try:
        df = pd.read_csv('../Datasets/PPR-ALL.csv')
    except OSError as e:
        logger.error(e)
        raise
